...
ROOT='data'
OUT=os.path.join(ROOT,'relations')
...
def norm(v):
    ...
    s=''.join(c for c in s if not unicodedata.combining(c)).lower().replace('ß','ss')
    return re.sub(r'[^a-z0-9]+',' ',s).strip()

def teamless(v):
    s=str(v or '').strip()
    s=re.sub(r'\s+(?:I{1,3}|IV|V|[1-5])\s*$','',s,flags=re.I)
    s=re.sub(r'\s+e\.?\s*v\.?\s*$','',s,flags=re.I)
    return s.strip()
...
for fi,path in enumerate(sorted(glob.glob(os.path.join(ROOT,'results-*.json'))),1):
    data=load_json(path); rows=arr(data,('results','ergebnisse','participations','teilnahmen','data'))
    for j,z in enumerate(rows):
        ...
        if club:
            ...
            clubs[cid]['result_ids'].append(rid); clubs[cid]['tournament_ids'].append(tid)
            if pid: clubs[cid]['player_ids'].append(pid)
        lv=str(first(z,('lv','association','verband','landesverband','LV')))
        if lv:
            aid=assoc_id(lv); associations.setdefault(aid,{'id':aid,'name':lv,'player_ids':[],'result_ids':[],'tournament_ids':[],'club_ids':[]})
            associations[aid]['result_ids'].append(rid); associations[aid]['tournament_ids'].append(tid)
